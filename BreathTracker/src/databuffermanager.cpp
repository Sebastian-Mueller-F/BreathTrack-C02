...
DataBufferManager::DataBufferManager(QObject *parent) : QObject(parent) {
  ...
  initializeBuffers();
  //
  _rawTrendBufferSubscription =
      std::make_shared<BufferSubscription>(*_buffers[SensorDataType::RAW]);
  _smaTrendBufferSubscription =
      std::make_shared<BufferSubscription>(*_buffers[SensorDataType::SMA]);
  _emaTrendBufferSubscription =
  ...
  _rawTrendBufferSubscription->registerSubscriber(trendFrontend, 50000,
                                                  SensorDataType::RAW);
  _smaTrendBufferSubscription->registerSubscriber(trendFrontend, 50000,
                                                  SensorDataType::SMA);
  _emaTrendBufferSubscription->registerSubscriber(trendFrontend, 50000,
                                                  SensorDataType::EMA);
}
...
void DataBufferManager::onNewData(double newData,
                                  SensorDataType sensorDataType) {
  // qDebug()<<" New Data for Data Buffer from " << sensorDataType << ", data: "
  // << newData;
...
void DataBufferManager::printBufferValues() {
  ...
  std::vector<double> rawValues =
      _buffers[SensorDataType::RAW]->readLastNValues(100);
  ...
  std::vector<double> smaValues =
      _buffers[SensorDataType::SMA]->readLastNValues(100);
  ...
  std::vector<double> emaValues =
      _buffers[SensorDataType::EMA]->readLastNValues(100);
