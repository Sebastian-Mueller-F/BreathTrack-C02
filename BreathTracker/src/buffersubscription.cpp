...
BufferSubscription::BufferSubscription(CircularBuffer &buffer, QObject *parent)
    : QObject(parent), _buffer(buffer) {
  bool connectionSuccess = connect(&_buffer, &CircularBuffer::dataAdded, this,
  ...
  for (size_t i = 0; i < _subscribers.size(); ++i) {
    try {
      ...
      std::cerr << "Out of range error: " << e.what()
                << " for subscriber index " << i << std::endl;
    } catch (const std::invalid_argument &e) {
      // Handle invalid argument errors (e.g., negative lookback period)
      std::cerr << "Invalid argument: " << e.what() << " for subscriber index "
                << i << std::endl;
    } catch (const std::runtime_error &e) {
      // Handle runtime errors (e.g., empty buffer, null subscriber)
      std::cerr << "Runtime error: " << e.what() << " for subscriber index "
                << i << std::endl;
    } catch (const std::exception &e) {
      // Catch any other standard exceptions
      std::cerr << "Exception: " << e.what() << " for subscriber index " << i
                << std::endl;
    } catch (...) {
      // Catch any other non-standard exceptions
      std::cerr << "Unknown error occurred for subscriber index " << i
                << std::endl;
    }
  }
}
