...
BufferSubscription::BufferSubscription(CircularBuffer &buffer, QObject *parent)
    ...
    , _buffer(buffer)
{
    qDebug() << "BufferSubscription created at address:" << this
             << "with CircularBuffer at address:" << &_buffer;
    bool connectionSuccess = connect(&_buffer,
                                     &CircularBuffer::dataAdded,
                                     this,
                                     &BufferSubscription::onDataAddedToBuffer);
...
{
    qDebug() << "BufferSubscription destroyed at address:" << this;
...
void BufferSubscription::onDataAddedToBuffer(int dataIntervalsMS) {
    ...
    for (size_t i = 0; i < _subscribers.size(); ++i) {
        try {
            ...
            // Handle out-of-range errors (e.g., accessing invalid indices or buffer
            // overflow)
            std::cerr << "Out of range error: " << e.what() << " for subscriber index " << i
                      << std::endl;
        } catch (const std::invalid_argument &e) {
            // Handle invalid argument errors (e.g., negative lookback period)
            std::cerr << "Invalid argument: " << e.what() << " for subscriber index " << i
                      << std::endl;
        } catch (const std::runtime_error &e) {
            // Handle runtime errors (e.g., empty buffer, null subscriber)
            std::cerr << "Runtime error: " << e.what() << " for subscriber index " << i
                      << std::endl;
